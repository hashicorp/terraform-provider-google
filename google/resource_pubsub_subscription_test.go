...
func TestAccPubsubSubscription_fullName(t *testing.T) {
	...
	topic := fmt.Sprintf("projects/%s/topics/tf-test-topic-%s", getTestProjectFromEnv(), acctest.RandString(10))
...
func TestAccPubsubSubscription_update(t *testing.T) {
	...
	topicLong := fmt.Sprintf("projects/%s/topics/tf-test-topic-%s", getTestProjectFromEnv(), acctest.RandString(10))
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccPubsubSubscription_fullName(topicLong, subscriptionLong, "bar", 20),
			},
			{
				ResourceName:      "google_pubsub_subscription.foo",
			...
			{
				Config: testAccPubsubSubscription_fullName(topicLong, subscriptionLong, "baz", 30),
			...
			{
				Config: testAccPubsubSubscription_fullName(topicLong, subscriptionShort, "baz", 30),
				Check: resource.TestCheckResourceAttr(
					"google_pubsub_subscription.foo", "path", subscriptionLong,
				),
			},
