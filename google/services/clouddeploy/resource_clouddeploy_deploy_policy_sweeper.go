...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionClouddeployDeployPolicy,
		DeleteFunction: testSweepClouddeployDeployPolicy,
	}
...
func testSweepClouddeployDeployPolicy(_ string) error {
	return listAndActionClouddeployDeployPolicy(deleteResourceClouddeployDeployPolicy)
}

func listAndActionClouddeployDeployPolicy(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ClouddeployDeployPolicy"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://clouddeploy.googleapis.com/v1/projects/{{project}}/locations/{{location}}/deployPolicies", "?")[0]
		...
		resourceList, ok := res["deployPolicies"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'deployPolicies'")
		} else {
...
func deleteResourceClouddeployDeployPolicy(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://clouddeploy.googleapis.com/v1/projects/{{project}}/locations/{{location}}/deployPolicies/{{name}}"
