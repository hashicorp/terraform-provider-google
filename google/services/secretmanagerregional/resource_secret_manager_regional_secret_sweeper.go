...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionSecretManagerRegionalRegionalSecret,
		DeleteFunction: testSweepSecretManagerRegionalRegionalSecret,
	}
...
func testSweepSecretManagerRegionalRegionalSecret(_ string) error {
	return listAndActionSecretManagerRegionalRegionalSecret(deleteResourceSecretManagerRegionalRegionalSecret)
}

func listAndActionSecretManagerRegionalRegionalSecret(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://secretmanager.{{location}}.rep.googleapis.com/v1/projects/{{project}}/locations/{{location}}/secrets", "?")[0]
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'regionalSecrets'")
		} else {
...
func deleteResourceSecretManagerRegionalRegionalSecret(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://secretmanager.{{location}}.rep.googleapis.com/v1/projects/{{project}}/locations/{{location}}/secrets/{{secret_id}}"
