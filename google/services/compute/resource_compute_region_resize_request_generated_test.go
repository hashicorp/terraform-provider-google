...
func TestAccComputeRegionResizeRequest_computeRmigResizeRequestExample(t *testing.T) {
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccComputeRegionResizeRequest_computeRmigResizeRequestExample(context),
			},
			{
				ResourceName:            "google_compute_region_resize_request.a3_resize_request",
				ImportState:             true,
				ImportStateVerify:       true,
				ImportStateVerifyIgnore: []string{"instance_group_manager", "region"},
			},
			{
				ResourceName:       "google_compute_region_resize_request.a3_resize_request",
				RefreshState:       true,
...
func testAccCheckComputeRegionResizeRequestDestroyProducer(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			if rs.Type != "google_compute_region_resize_request" {
				continue
			}
			...
			url, err := tpgresource.ReplaceVarsForTest(config, rs, transport_tpg.BaseUrl(compute.Product, config)+"projects/{{project}}/regions/{{region}}/instanceGroupManagers/{{instance_group_manager}}/resizeRequests/{{name}}")
			if err != nil {
				return err
			}
			...
			if err == nil {
				return fmt.Errorf("ComputeRegionResizeRequest still exists at %s", url)
			}
		}
