...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionBlockchainNodeEngineBlockchainNodes,
		DeleteFunction: testSweepBlockchainNodeEngineBlockchainNodes,
	}
...
func testSweepBlockchainNodeEngineBlockchainNodes(_ string) error {
	return listAndActionBlockchainNodeEngineBlockchainNodes(deleteResourceBlockchainNodeEngineBlockchainNodes)
}

func listAndActionBlockchainNodeEngineBlockchainNodes(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "BlockchainNodeEngineBlockchainNodes"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://blockchainnodeengine.googleapis.com/v1/projects/{{project}}/locations/{{location}}/blockchainNodes", "?")[0]
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'blockchainNodes'")
		} else {
...
func deleteResourceBlockchainNodeEngineBlockchainNodes(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://blockchainnodeengine.googleapis.com/v1/projects/{{project}}/locations/{{location}}/blockchainNodes/{{blockchain_node_id}}"
