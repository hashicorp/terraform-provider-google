...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionEdgecontainerVpnConnection,
		DeleteFunction: testSweepEdgecontainerVpnConnection,
	}
...
func testSweepEdgecontainerVpnConnection(_ string) error {
	return listAndActionEdgecontainerVpnConnection(deleteResourceEdgecontainerVpnConnection)
}

func listAndActionEdgecontainerVpnConnection(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'vpnConnections'")
		} else {
...
func deleteResourceEdgecontainerVpnConnection(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
