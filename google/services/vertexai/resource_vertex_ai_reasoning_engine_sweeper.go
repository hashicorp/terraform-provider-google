...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionVertexAIReasoningEngine,
		DeleteFunction: testSweepVertexAIReasoningEngine,
	}
...
func testSweepVertexAIReasoningEngine(_ string) error {
	return listAndActionVertexAIReasoningEngine(deleteResourceVertexAIReasoningEngine)
}

func listAndActionVertexAIReasoningEngine(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "VertexAIReasoningEngine"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://{{region}}-aiplatform.googleapis.com/v1/projects/{{project}}/locations/{{region}}/reasoningEngines", "?")[0]
		...
		resourceList, ok := res["reasoningEngines"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'reasoningEngines'")
		} else {
...
func deleteResourceVertexAIReasoningEngine(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://{{region}}-aiplatform.googleapis.com/v1/projects/{{project}}/locations/{{region}}/reasoningEngines/{{name}}"
