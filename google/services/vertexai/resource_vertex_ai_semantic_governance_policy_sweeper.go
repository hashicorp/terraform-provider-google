...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionVertexAISemanticGovernancePolicy,
		DeleteFunction: testSweepVertexAISemanticGovernancePolicy,
	}
...
func testSweepVertexAISemanticGovernancePolicy(_ string) error {
	return listAndActionVertexAISemanticGovernancePolicy(deleteResourceVertexAISemanticGovernancePolicy)
}

func listAndActionVertexAISemanticGovernancePolicy(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "VertexAISemanticGovernancePolicy"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://{{region}}-aiplatform.googleapis.com/v1/projects/{{project}}/locations/{{region}}/semanticGovernancePolicies", "?")[0]
		...
		resourceList, ok := res["semanticGovernancePolicies"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'semanticGovernancePolicies'")
		} else {
...
func deleteResourceVertexAISemanticGovernancePolicy(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://{{region}}-aiplatform.googleapis.com/v1/projects/{{project}}/locations/{{region}}/semanticGovernancePolicies/{{semantic_governance_policy_id}}"
