...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionVertexAIFeatureGroup,
		DeleteFunction: testSweepVertexAIFeatureGroup,
	}
...
func testSweepVertexAIFeatureGroup(_ string) error {
	return listAndActionVertexAIFeatureGroup(deleteResourceVertexAIFeatureGroup)
}

func listAndActionVertexAIFeatureGroup(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "VertexAIFeatureGroup"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://{{region}}-aiplatform.googleapis.com/v1/projects/{{project}}/locations/{{region}}/featureGroups", "?")[0]
		...
		resourceList, ok := res["featureGroups"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'featureGroups'")
		} else {
...
func deleteResourceVertexAIFeatureGroup(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://{{region}}-aiplatform.googleapis.com/v1/projects/{{project}}/locations/{{region}}/featureGroups/{{name}}"
