...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionTranscoderJob,
		DeleteFunction: testSweepTranscoderJob,
	}
...
func testSweepTranscoderJob(_ string) error {
	return listAndActionTranscoderJob(deleteResourceTranscoderJob)
}

func listAndActionTranscoderJob(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "TranscoderJob"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://transcoder.googleapis.com/v1/projects/{{project}}/locations/{{location}}/jobs", "?")[0]
...
func deleteResourceTranscoderJob(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://transcoder.googleapis.com/v1/{{name}}"
