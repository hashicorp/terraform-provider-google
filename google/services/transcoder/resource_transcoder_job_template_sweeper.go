...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionTranscoderJobTemplate,
		DeleteFunction: testSweepTranscoderJobTemplate,
	}
...
func testSweepTranscoderJobTemplate(_ string) error {
	return listAndActionTranscoderJobTemplate(deleteResourceTranscoderJobTemplate)
}

func listAndActionTranscoderJobTemplate(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "TranscoderJobTemplate"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://transcoder.googleapis.com/v1/projects/{{project}}/locations/{{location}}/jobTemplates", "?")[0]
		...
		resourceList, ok := res["jobTemplates"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'jobTemplates'")
		} else {
...
func deleteResourceTranscoderJobTemplate(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://transcoder.googleapis.com/v1/projects/{{project}}/locations/{{location}}/jobTemplates/{{job_template_id}}"
