...
func TestAccContactCenterInsightsViewListQuery_generated(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_contact_center_insights_view.basic_view",
						[]string{
							"name",
						},
					),
					listScope.Capture(map[string]string{
						"location": "google_contact_center_insights_view.basic_view",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccContactCenterInsightsView_contactCenterInsightsViewBasicExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_contact_center_insights_view.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_contact_center_insights_view.list_query",
...
func testAccContactCenterInsightsView_contactCenterInsightsViewBasicExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "location" { type = string }
list "google_contact_center_insights_view" "list_query" {
    provider = google
	limit = 10000
    config {
