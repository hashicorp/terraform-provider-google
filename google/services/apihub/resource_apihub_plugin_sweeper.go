...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionApihubPlugin,
		DeleteFunction: testSweepApihubPlugin,
	}
...
func testSweepApihubPlugin(_ string) error {
	return listAndActionApihubPlugin(deleteResourceApihubPlugin)
}

func listAndActionApihubPlugin(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ApihubPlugin"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://apihub.googleapis.com/v1/projects/{{project}}/locations/{{location}}/plugins", "?")[0]
		...
		resourceList, ok := res["plugins"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'plugins'")
		} else {
...
func deleteResourceApihubPlugin(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://apihub.googleapis.com/v1/projects/{{project}}/locations/{{location}}/plugins/{{plugin_id}}"
