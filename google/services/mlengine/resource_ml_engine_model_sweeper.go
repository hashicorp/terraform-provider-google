...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionMLEngineModel,
		DeleteFunction: testSweepMLEngineModel,
	}
...
func testSweepMLEngineModel(_ string) error {
	return listAndActionMLEngineModel(deleteResourceMLEngineModel)
}

func listAndActionMLEngineModel(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'models'")
		} else {
...
func deleteResourceMLEngineModel(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
