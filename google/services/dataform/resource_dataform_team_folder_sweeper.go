...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionDataformTeamFolder,
		DeleteFunction: testSweepDataformTeamFolder,
	}
...
func testSweepDataformTeamFolder(_ string) error {
	return listAndActionDataformTeamFolder(deleteResourceDataformTeamFolder)
}

func listAndActionDataformTeamFolder(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "DataformTeamFolder"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://dataform.googleapis.com/v1/", "?")[0]
		...
		resourceList, ok := res["teamFolders"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'teamFolders'")
		} else {
...
func deleteResourceDataformTeamFolder(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://dataform.googleapis.com/v1/projects/{{project}}/locations/{{region}}/teamFolders/{{teamfolder_id}}"
