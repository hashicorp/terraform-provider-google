...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionEventarcTrigger,
		DeleteFunction: testSweepEventarcTrigger,
	}
...
func testSweepEventarcTrigger(_ string) error {
	return listAndActionEventarcTrigger(deleteResourceEventarcTrigger)
}

func listAndActionEventarcTrigger(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://eventarc.googleapis.com/v1/projects/{{project}}/locations/{{location}}/triggers", "?")[0]
...
func deleteResourceEventarcTrigger(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://eventarc.googleapis.com/v1/projects/{{project}}/locations/{{location}}/triggers/{{name}}"
