...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionBigqueryDatapolicyv2DataPolicy,
		DeleteFunction: testSweepBigqueryDatapolicyv2DataPolicy,
	}
...
func testSweepBigqueryDatapolicyv2DataPolicy(_ string) error {
	return listAndActionBigqueryDatapolicyv2DataPolicy(deleteResourceBigqueryDatapolicyv2DataPolicy)
}

func listAndActionBigqueryDatapolicyv2DataPolicy(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "BigqueryDatapolicyv2DataPolicy"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://bigquerydatapolicy.googleapis.com/v2/projects/{{project}}/locations/{{location}}/dataPolicies", "?")[0]
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'dataPolicies'")
		} else {
...
func deleteResourceBigqueryDatapolicyv2DataPolicy(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://bigquerydatapolicy.googleapis.com/v2/projects/{{project}}/locations/{{location}}/dataPolicies/{{data_policy_id}}"
