...
func testAccCheckAppEngineFirewallRuleDestroyProducer(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			...
			url, err := tpgresource.ReplaceVarsForTest(config, rs, fmt.Sprintf("%s%s", transport_tpg.BaseUrl(appengine.Product, config), "apps/{{project}}/firewall/ingressRules/{{priority}}"))
			if err != nil {
				return err
			}
