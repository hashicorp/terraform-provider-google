...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkServicesMulticastGroupRange,
		DeleteFunction: testSweepNetworkServicesMulticastGroupRange,
	}
...
func testSweepNetworkServicesMulticastGroupRange(_ string) error {
	return listAndActionNetworkServicesMulticastGroupRange(deleteResourceNetworkServicesMulticastGroupRange)
}

func listAndActionNetworkServicesMulticastGroupRange(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkServicesMulticastGroupRange"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networkservices.googleapis.com/v1/projects/{{project}}/locations/{{location}}/multicastGroupRanges", "?")[0]
		...
		resourceList, ok := res["multicastGroupRanges"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'multicastGroupRanges'")
		} else {
...
func deleteResourceNetworkServicesMulticastGroupRange(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networkservices.googleapis.com/v1/projects/{{project}}/locations/{{location}}/multicastGroupRanges/{{multicast_group_range_id}}"
