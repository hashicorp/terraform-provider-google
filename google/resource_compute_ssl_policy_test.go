		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckComputeSslPolicyCustomFeatures(&sslPolicy, "TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384"),
				),
			},
			resource.TestStep{
...
func testAccCheckComputeSslPolicyCustomFeatures(sslPolicy *computeBeta.SslPolicy, n string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		if sslPolicy.CustomFeatures == nil {
			return fmt.Errorf("no custom_features")
		}
		...
		return fmt.Errorf("custom_features not found: %s", n)
	}
}
