...
func performZoneUpdate(d *schema.ResourceData, config *Config, hasVersions bool) error {
	...
	if updateStrategy == "ROLLING_UPDATE" {
		...
		if err != nil {
			return fmt.Errorf("error retrieving instance group manager: %s", err.Error())
		}
		manager := &computeBeta.InstanceGroupManager{
			Name:                d.Get("name").(string),
			Description:         d.Get("description").(string),
		...
		if hasVersions {
			manager.Versions = expandVersions(d.Get("version").([]interface{}))
		} else {
			manager.InstanceTemplate = d.Get("instance_template").(string)
		}
...
func resourceComputeInstanceGroupManagerUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	// the GCP API will automatically create a version for us if we only supply an instance template
	// that version then gets set in state
	// then future calls to Update think that version is something the user specified, not something that GCP autogenerated
	// and the GCP API ignores instance templates in favor of versions
	// so by default, you can't actually update the instance template an instance group manager is using while using rolling deploys
	// (non-rolling deploys can use patch, so they're fine. Rolling deploys must use put, and specify the whole resource, including
	// any versions, which is where we get into trouble.)
	// This works around that issue in a gross, ugly way.
	// If we have only one version defined, and the version uses the same instance template our instance group manager has
	// as its instance_template in state
	// then we're going to ignore the versions in the update, because that's the version that GCP autogenerated for us.
	versions := expandVersions(d.Get("version").([]interface{}))
	oldTemplate, _ := d.GetChange("instance_template")
	defaultVersionOnly := len(versions) == 1 && versions[0].Name == "" &&
		(versions[0].TargetSize == nil || (versions[0].TargetSize.Fixed == 0 && versions[0].TargetSize.Percent == 0)) &&
		compareSelfLinkRelativePaths("", versions[0].InstanceTemplate, oldTemplate.(string), nil)
	...
	if d.HasChange("instance_template") {
		...
		err = performZoneUpdate(d, config, !defaultVersionOnly)
		if err != nil {
			return err
		}
